param([Parameter(Mandatory)]$Classification,[Parameter(Mandatory)][string]$SourceName,[string]$Vault=(Split-Path $PSScriptRoot -Parent))
...
foreach($e in @($Classification.entities)){if(!$e){continue};$p=Join-Path $entityRoot ("$(Norm "$e").md");if(!(Test-Path $p)){"# $e`n"|Out-File $p -Encoding utf8};Add-Content $p "- [[$SourceName]]" -Encoding utf8}
