...
function renderInteractiveAI(runtime) {
  ...
  if (openProcessor) {
    ...
    if (runtime.wan2gp?.url) openProcessor.href = runtime.wan2gp.url;
...
async function launchWan2GP(action) {
  ...
  try {
    ...
    if (action === "start") {
      const renderer = await waitForWan2GP();
      const rendererUrl = renderer?.url || "http://127.0.0.1:8766/";
      if (rendererWindow && !rendererWindow.closed) {
        rendererWindow.location.href = rendererUrl;
      } else {
        window.location.href = rendererUrl;
      }
    }
    await refresh();
  } catch (error) {
